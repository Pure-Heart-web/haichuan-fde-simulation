...
这是一个可复现的模拟客户项目。目前完成 **Stage 1–3 的业务与数据设计资料、Stage 4 的询盘抽取、Stage 5 的产品匹配，以及 Stage 6 的客户上下文、企业知识和证据化回复草稿教学切片**。后续阶段按新需求继续扩展。
...
`all` 依次运行六个阶段：Stage 1 生成六项 Discovery 交付物，Stage 2/3 生成资料与数据审计报告，Stage 4 将 20 封合成邮件导入审核队列并运行 100 条合成评估，Stage 5 对 24 条合成产品匹配案例运行推荐与评估，Stage 6 对客户身份、知识检索和回复草稿生成评估。结果保存在 `outputs/stage-01/` 至 `outputs/stage-06/`。已有人工审核不会因重复运行而覆盖；动态报告会更新。
...
Stage 1–3 资料可用任意 Markdown 阅读器浏览；Stage 4–6 的人工复核需启动各自本地页面。需要修改行为时，从 [Stage 1 命令入口](run.py)、[Stage 4 命令入口](stage4.py)、[Stage 5 命令入口](stage5.py)和 [Stage 6 命令入口](stage6.py)阅读；这套工具没有调用外部大模型，生成结果可以直接追溯到本地输入。
...
六个阶段的“交付”是可追溯的问题定义、治理决定与可运行的抽取、推荐、证据化草稿和审核切片。Stage 4 使用离线正则基线，合成样本总体字段准确率 96.7%，但流量和温度均为 90%，未达到每字段至少 95% 的设计门槛。Stage 5 的 24 条合成匹配集有 2 条误升级；Stage 6 的身份、检索和草稿高分来自小型自造样本。没有真实模型、性能曲线、客户销售现场测试、邮件自动发送或实际 Pilot 成效。
