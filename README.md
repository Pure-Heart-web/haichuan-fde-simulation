...
这是一个可复现的模拟客户项目。目前完成 **Stage 1–3 的业务与数据设计资料、Stage 4 的询盘抽取与审核，以及 Stage 5 的产品匹配与人工复核教学切片**。后续阶段按新需求继续扩展。
...
`all` 依次运行五个阶段：Stage 1 生成六项 Discovery 交付物，Stage 2/3 生成资料与数据审计报告，Stage 4 将 20 封合成邮件导入审核队列并运行 100 条合成评估，Stage 5 对 24 条合成产品匹配案例运行推荐与评估。结果保存在 `outputs/stage-01/` 至 `outputs/stage-05/`。已有人工审核不会因重复运行而覆盖；动态报告会更新。
...
Stage 1–3 资料可用任意 Markdown 阅读器浏览；Stage 4/5 的人工复核需启动各自本地页面。需要修改行为时，从 [Stage 1 命令入口](run.py)、[Stage 4 命令入口](stage4.py)和 [Stage 5 命令入口](stage5.py)阅读；这套工具没有调用外部大模型，生成结果可以直接追溯到本地输入。
...
src/fde_platform/             Sprint 1/2 抽取、推荐、人工审核与评估代码
scripts/                      合成资料与评估集重建脚本
tests/                        指标计算、无效输入与阶段边界测试
run.py                        命令行入口
stage_pipeline.py             五阶段离线检查入口
...
五个阶段的“交付”是可追溯的问题定义、治理决定与可运行的抽取、推荐和审核切片。Stage 4 当前使用离线正则基线，合成样本总体字段准确率 96.7%，但流量和温度均为 90%，未达到每字段至少 95% 的设计门槛。Stage 5 的 24 条合成匹配集有 2 条误升级；其 100% 候选召回只是教学样本结果。没有真实模型、性能曲线、客户销售现场测试或实际 Pilot 成效。
