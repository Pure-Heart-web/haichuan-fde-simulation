...
这是一个可复现的模拟客户项目。目前完成 **Stage 1–7 的海川工业泵发现、构建和试点运营演练，Stage 8 的启航设备售后与双客户产品化，Stage 9–14 的交付强化、客户接入、运行、模型、现场与集成控制，Stage 15 的商务交付 Gate，Stage 16 的团队商业交付毕业模拟，Stage 17 的付费 Discovery 合同到验收，Stage 18 的 Design Partner 获取与签约预检，Stage 19 的付费 Discovery 现场执行，以及 Stage 20 的客户验收、Pilot 投资与启动门**。后续阶段按新需求继续扩展。
...
```bash
python3 stage_pipeline.py all --output-root outputs/my-stage-1-20-run
python3 -m unittest discover -s tests -v
```

`--output-root` 建议每次使用新目录；历史审核记录与当前代码版本不同时，流水线会按设计拒绝覆盖。

`all` 依次运行二十个阶段：Stage 1–3 生成 Discovery 与数据审计，Stage 4–8 完成抽取、推荐、知识、Pilot 和双客户产品化，Stage 9–14 检查交付强化、客户接入、运行、模型、现场与集成控制，Stage 15 校验商务交易档案与 A–F Gate，Stage 16 评分团队和个人技术责任证据，Stage 17–19 推进付费 Discovery 的合同、Design Partner 和现场执行，Stage 20 校验客户验收、整改、Steering 投资决定、开票条件、Pilot SOW 和 Mobilization。结果保存在 `outputs/stage-01/` 至 `outputs/stage-20/`，另有 `outputs/stage-08-vertical/`。
...
二十个阶段的“交付”是可追溯的问题定义、治理决定与可运行的技术、运营、商务和团队责任演练。Stage 14 依然使用本地集成 Lab；Stage 15–20 的公开交易、合同、客户事件、金额、人员、验收、投资决定和签核均为教学虚构。Stage 18–20 支持仓库外真实记录的结构预检，但代码不能代替授权客户现场测试、真实 Pilot 成效、有效合同、真实发票/回款或客户验收。
