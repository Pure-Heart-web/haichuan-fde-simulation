...
def main():
    ...
    parser.add_argument('stage', choices=['stage1', 'stage2', 'stage3', 'stage4', 'stage5', 'stage6', 'stage7', 'stage8', 'stage9', 'stage10', 'stage11', 'stage12', 'stage13', 'stage14', 'stage15', 'stage16', 'stage17', 'stage18', 'stage19', 'stage20', 'stage21', 'stage22', 'all'])
    ...
    try:
        ...
        if args.stage in ('stage14', 'all'):
            from stage14 import run_demo
            run_demo((args.output_root / 'stage-14').resolve(), check_baseline=True)
        if args.stage in ('stage15', 'all'):
            from stage15 import run_demo
            run_demo((args.output_root / 'stage-15').resolve(), check=True)
        if args.stage in ('stage16', 'all'):
            from stage16 import run_demo
            run_demo((args.output_root / 'stage-16').resolve(), check=True)
        if args.stage in ('stage17', 'all'):
            from stage17 import run_demo
            run_demo((args.output_root / 'stage-17').resolve(), check=True)
        if args.stage in ('stage18', 'all'):
            from stage18 import run_demo
            run_demo((args.output_root / 'stage-18').resolve(), check=True)
        if args.stage in ('stage19', 'all'):
            from stage19 import run_demo
            run_demo((args.output_root / 'stage-19').resolve(), check=True)
        ...
        if args.stage in ('stage22', 'all'):
            from stage22 import run_demo
            run_demo((args.output_root / 'stage-22').resolve(), check=True)
        return 0
